...
for(i in seq_along(r_func_vec)) {
  source(paste0("main/code/functions/", r_func_vec[i], ".R"))
}
...
for(i in seq_along(p_miss_vec)) {
  ...
  file_name = paste0("main/data/", scenario, "_p", str_replace(string = as.character(p_miss), pattern = "\\.", replacement = ""), ".csv")
